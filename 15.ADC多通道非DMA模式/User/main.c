...
int main(void)
{
	...
	while (1)
	{
		AD0 = AdGetValue(ADC_Channel_0);		//单次启动ADC，转换通道0
		AD1 = AdGetValue(ADC_Channel_1);		//单次启动ADC，转换通道1
		AD2 = AdGetValue(ADC_Channel_2);		//单次启动ADC，转换通道2
		AD3 = AdGetValue(ADC_Channel_3);		//单次启动ADC，转换通道3
